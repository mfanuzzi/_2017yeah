// There are two small modifications made to this (and in the .h to match.)
// One is to ignore custom pixelOffset function values of 666 (line 150).
// (^ This allows for handling rotation without wrapping in the remap function.)
// The other is the addition of remapFn2, which lets you modify the x,y of a
// pixel, while still utilizing the library's handling of orientation, etc.
...
Adafruit_DotStarMatrix_MF::Adafruit_DotStarMatrix_MF(int w, int h,
...
Adafruit_DotStarMatrix_MF::Adafruit_DotStarMatrix_MF(uint8_t mW, uint8_t mH,
...
uint16_t Adafruit_DotStarMatrix_MF::Color(uint8_t r, uint8_t g, uint8_t b) {
...
void Adafruit_DotStarMatrix_MF::setPassThruColor(uint32_t c) {
  passThruColor = c;
  passThruFlag  = true;
}

// Call without a value to reset (disable passthrough)
void Adafruit_DotStarMatrix_MF::setPassThruColor(void) {
  passThruFlag = false;
}

void Adafruit_DotStarMatrix_MF::drawPixel(int16_t x, int16_t y, uint16_t color) {
...
void Adafruit_DotStarMatrix_MF::fillScreen(uint16_t color) {
  uint16_t i, n;
  uint32_t c;
...
void Adafruit_DotStarMatrix_MF::setRemapFunction(uint16_t (*fn)(uint16_t, uint16_t)) {
  remapFn = fn;
}

void Adafruit_DotStarMatrix_MF::setRemapFunction2(void (*fn)(int16_t&, int16_t&)) {
  remapFn2 = fn;
}
